...
def _max_capacity() -> int:
    ...
    If none is configured, the max capacity defaults to 1.
    An explicit value of 0 means no job is dispatched at all.
    """
    ...
    def load_channels_config(self):
        ...
        with closing(self.conn.cursor()) as cr:
            cr.execute(
                ...
                "COALESCE(capacity_default, 0), "
                "COALESCE(sequential_default, false) "
            ...
            configs = [
                ChannelConfig(
                    ...
                    sequential_default=sequential_default,
                )
                for (
                    name,
                    capacity,
...
class QueueJobRunner:
    def __init__(
        ...
        self.max_capacity = max(0, max_capacity)
    ...
    def _build_channel_manager(self, db):
        ...
        try:
            ...
            # break other databases, use a paused root channel
            # instead. If the configuration is corrected on the DB,
            # the next reload should rebuild the channel manager.
            _logger.warning(
                "invalid channel configuration for db %s, pausing until resolution",
                db.db_name,
                exc_info=True,
            )
            return self._create_paused_root_channel_manager(db_name=db.db_name)
    ...
    def run_jobs(self):
        ...
        while not self._stop:
            if self._all_running_count() >= self.max_capacity:
                _logger.debug(
    ...
    def next_wakeup_time(self):
        ...
        # until we get new notifications (practically, set a SELECT_TIMEOUT,
        # and wait for notifications). Wake-ups of more than 0 are used to
        # dispatch jobs with ETA, as these will not notify anything when they
        # actually have to start.
        # So when getting the next wake-up time, we find the first channel
        # manager with ETA jobs (a wake-up time more than 0), and if none,
        # a wake-up time of zero is returned to go back to the select waiting
        # for notifications.
        wakeup_times = [
            wakeup_time
            for channel_manager in self._channel_managers
            if (wakeup_time := channel_manager.get_wakeup_time())
        ]
